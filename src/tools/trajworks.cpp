...
{ bool ttutto=false;
  ...
  return (regex=="*")||(label==regex)||(ttutto); }
#endif
...
void pgalign(const std::vector<AtomData>& rfr, std::vector<AtomData>& asel, std::vector<AtomData>& bsel)
{   // rfr: the ref wat molecule
    // asel : the index of the wat molec to align 
    // bsel : the w molecules of a certain species around asel
    
    FMatrix<double> rmat;
    std::vector<AtomData> ref=rfr;
    
    double dx,dy,dz;
    dx=dy=dz=0.0;
    //first center the reference structure.
    //computes COM of the ref and the tagged mol
    AtomData rcom,pcom;
    rcom.x=rcom.y=rcom.z=pcom.x=pcom.y=pcom.z=0.0;
    for (int i=0; i<ref.size(); ++i){
        rcom.x+=ref[i].x; rcom.y+=ref[i].y; rcom.z+=ref[i].z;
        pcom.x+=asel[i].x; pcom.y+=asel[i].y; pcom.z+=asel[i].z;
    }
    rcom.x/=3.0; rcom.y/=3.0; rcom.z/=3.0;
    pcom.x/=3.0; pcom.y/=3.0; pcom.z/=3.0;
    
    // get the distance between the ref mol and the tagged one 
    dx=pcom.x-rcom.x;  
    dy=pcom.y-rcom.y;  
    dz=pcom.z-rcom.z;
    //center the reference structure to zero
    for (int i=0; i<ref.size(); ++i) {
        ref[i].x-=rcom.x;  
        ref[i].y-=rcom.y;  
        ref[i].z-=rcom.z;
    }
    //shif the tagged mol to zero
    ...
    // minimaze and rotate the tagged mol on the ref struct
    // and get rmatr
    micrmsd(ref,asel,rmat);
    // rotate
    for (int i=0; i<asel.size(); ++i){
        micmat(rmat, asel[i].x, asel[i].y, asel[i].z);        
    }
    // 4. test it
    // 5. rotate the oxygens wm2
    for (int i=0; i<bsel.size(); ++i){
        micmat(rmat, bsel[i].x, bsel[i].y, bsel[i].z);        
    }
...
void banner()
{
    std::cerr
            ...
            << " ## 3d WATER density OPTIONS:  activate by -wdens                               \n"
            << " -wddat1    label of the monitored specie [*]                                   \n"
            << " -wddat2    label of the monitored specie [*]                                   \n"
            << " -wdref    laboratory water molecule                                            \n"
            << " -wdcutoff cutoff for the analysis                                            \n"
            ...
            << " ## mean square displacement calculation (-msd) or (-msdsingle)                 \n"
            << " -msdat    label of the monitored specie [*]                                    \n"
            << " -msdlag   maximum time-lag to compute msd for [1000]                           \n"
            << " ## compute the msd dstribution    (-msdbrown)                                   \n"
            << " -msdbrownrange range for the histogram                                          \n"
...
int main(int argc, char **argv)
{
    ...
    bool fwdens=false, fgdr2=false, fgdr=false, fpos=false, fxtc2xyz=false, fvvac=false, fxyz=false, fpdb=false , fdlp=false, fxtc=false, fmsd=false, fmsdsingle=false,fmsdbr=false, fdipole=false, fdens=false, fdtraj=false, fdproj=false, fdpov=false, fpca=false, fpcaxyz=false, fpcanocov=false, fvbox=false, fcv=false, fpd=false, fvvacbox=false, fp3d=false, fp3dinv=false, funwrap=false, fpproj=false, fthermal=false, fcharge=false, fhelp;
    std::string fwref, lwd1, lwd2, lgdr1, lgdr2, ltype, lrat, dummy, lvvac, lmsd, ldens, ldalign, lpcalign, lpcat, prefix, fbox, fqat, sdbins, sdfold, sdrange, fref, lpdat,shwin, lp3dat, flab, lppat1, lppat2, lcv, fweights;
    double cogdr, dt, densw, pdmax, p3dmax, hwinfac; unsigned long fstart,fstop,fstep, msdbrrng, gdrbins, vvlag, msdlag, ftpad, dbinsx, dbinsy, dbinsz, dfoldx, dfoldy, dfoldz, cvtype, pdbins, p3dbins;
    double wdcutoff, drangeax, drangebx,  drangeay, drangeby,  drangeaz, drangebz;
    ...
    bool fok=
            ...
            clp.getoption(fgdr2,"gracf",false) &&            
            ...
            clp.getoption(fmsdsingle,"msdsingle",false) &&
            clp.getoption(msdlag,"msdlag",(unsigned long) 1000) &&
            clp.getoption(lmsd,"msdat",std::string("*")) &&
            clp.getoption(msdbrrng,"msdbrownrange",(unsigned long) 14) &&
            clp.getoption(fmsdbr,"msdbrown",false) &&
            //water density options
            clp.getoption(fwdens,"wdens",false) &&
            clp.getoption(lwd1,"wddat1",std::string("*")) &&
            clp.getoption(lwd2,"wddat2",std::string("*")) &&
            clp.getoption(fwref,"wdref",std::string("")) &&
            clp.getoption(wdcutoff,"wdcutoff",(double) 6.) &&
            //density options
    ...
    std::valarray<AtomFrame> msdbuff(msdlag); //AtomFrame refmsd;
    std::valarray<double> dmsd(msdlag); nmsd=0; dmsd=0.; std::valarray<bool> fmsd_inc;
    FMatrix<bool>  fmsd_inc_br(msdlag,msdlag);
    std::valarray<unsigned long> msdnat_br(msdlag);
    FMatrix<double> DMSDSINGLE(3,3); DMSDSINGLE*=0.;
    ...
    // parameters for sa and sd 1D
    // lenght msdbrrng, smoothing 0.025
    double abr=0, bbr=msdbrrng ; unsigned long nbinsbr=(msdbrrng/(0.05/2));
    std::valarray<HGOptions<Histogram<double> > > hgobr(2);
    hgobr[0].boundaries.resize((msdlag+1)*dt);
    hgobr[0].window_width=dt*0.5;
    hgobr[0].window=HGWTriangle;
    hgobr[0].walls=HGBHard;
    for (int k=0; k<=msdlag;k++) hgobr[0].boundaries[k]=dt*k;
    
    hgobr[1].boundaries.resize(nbinsbr);
    hgobr[1].window_width=0.05;
    hgobr[1].window=HGWTriangle;
    hgobr[1].walls=HGBHard;
    for (int k=0; k<nbinsbr;k++) hgobr[1].boundaries[k]=k*0.05/2;
    //hgbr.set_options(hgobr);
    NDHistogram<double> hgbr(hgobr);
            
    //thermal ellipsoids stuff
    FMatrix<double> te_u0, te_uiuj;

    //initializes output streams
    std::ostream *ogdr, *opos, *oxtc2xyz, *ocv, *ovvac, *omsd, *omsdbr, *odipole, *odens, *odtraj, *odpov, *opca, *opcaxyz, *opd, *op3d, *opproj, *otherm, *ocharge;
    ...
    else
    {
        ...
        if (fgdr2)
        {   ogdr=new std::ofstream((prefix+std::string(".gdr2")).c_str());
            (*ogdr).precision(8); (*ogdr).width(15); (*ogdr).setf(std::ios::scientific);
        }
        if (fpos)
        {
        ...
        if (fmsdbr)
        {
            omsdbr=new std::ofstream((prefix+std::string(".msdbrown")).c_str());
            (*omsdbr).precision(8); (*omsdbr).width(15); (*omsdbr).setf(std::ios::scientific);
            omsd=new std::ofstream((prefix+std::string(".msd")).c_str());
            (*omsd).precision(8); (*omsd).width(15); (*omsd).setf(std::ios::scientific);
        }
        ...
        if (fwdens)
        {
            odtraj=new std::ofstream((prefix+std::string(".xyz")).c_str());
    ...
    AtomFrame reffr;
    std::vector<AtomData> wmref;
    
    // Density water stuff
    std::vector<unsigned long> w1sel, w2sel;
    // read the ref wat molecule
    if (fwref!="")
    {   
        //reads reference frame from external file
        std::ifstream ifref(fwref.c_str()); bool succ;
        if (fxyz) succ=ReadXYZFrame(ifref,reffr);
        else ERROR("I don't know how to read this...");

        if (!succ) ERROR("Format error in ref file.");       
        
        // put the molecule in vctor of AtomicData variab
        AtomData wmtemp;
        
        for (int j=0; j<3;++j){
            wmtemp.x=wmtemp.y=wmtemp.z=0.0;
            wmtemp.x=reffr.ats[j].x;
            wmtemp.y=reffr.ats[j].y;
            wmtemp.z=reffr.ats[j].z;
            wmref.push_back(wmtemp);
        }                
    }
    ...
    while ((fdlp && ReadDLPFrame(std::cin,af))||(fxtc && ReadXTCFrame(stdin,af)) || (fxyz && ReadXYZFrame(std::cin,af))  || (fpdb && ReadPDBFrame(std::cin,af)) )
    {
        ...
        { 
            ...
            for( int i=0 ; i<af.ats.size(); ++i ) {
                af.ats[i].name=reffr.ats[i].name; 
                af.ats[i].atlabel=reffr.ats[i].atlabel;
                af.ats[i].molname=reffr.ats[i].molname; 
                af.ats[i].group=reffr.ats[i].group;
                af.ats[i].nprops["mol"]=reffr.ats[i].nprops["mol"];
            }      
        ...
        if (fwdens)
        {
           // std::cout<<lwd1<<" "<<lwd2<<" "<<reffr.ats[1].name<<" "<<reffr.ats[1].x<<" "<<reffr.ats[1].y<<" "<<reffr.ats[1].z<<"\n";
           // std::cout<<af.ats[1].name<<" "<<af.ats[1].x<<" "<<af.ats[1].y<<" "<<af.ats[1].z<<"\n";
           ...
           std::vector<AtomData> wmsel;
           // select the type 1 molecules
           for (int i=0; i<af.ats.size();++i)
                if(chk_sel(af.ats[i].name,lwd1)){
                    AtomData wmpbc;
                    double threeco2=wdcutoff*wdcutoff*3.0; 
                    //tagged wat mol
                    std::vector<AtomData>  wmtg;
                    for (int j=0; j<3;++j){
                        wmpbc.x=wmpbc.y=wmpbc.z=0.0;
                        wmpbc.x=af.ats[i+j].x;
                        wmpbc.y=af.ats[i+j].y;
                        wmpbc.z=af.ats[i+j].z;
                        wmtg.push_back(wmpbc);
                    }    
                    ...
                    for (int j=0; j<af.ats.size();++j){
                        ...
                        if(i!=j && chk_sel(af.ats[j].name,lwd2)){
                            ngdrpairs++; // keep track of ALL potential atom pairs
                            // check the cutoff (PBC) and select
                            wmpbc.x=af.ats[j].x-af.ats[i].x;
                            wmpbc.y=af.ats[j].y-af.ats[i].y;
                            wmpbc.z=af.ats[j].z-af.ats[i].z;
                            micmat(ICM,wmpbc.x,wmpbc.y,wmpbc.z);
                            micpbc(1.,1.,1.,wmpbc.x,wmpbc.y,wmpbc.z);
                            micmat(CM,wmpbc.x,wmpbc.y,wmpbc.z);
                            if ((wmpbc.x*wmpbc.x+wmpbc.y*wmpbc.y+wmpbc.z*wmpbc.z)>threeco2) continue;

                            // Apply the PBC
                          /*
                            wmpbc.x=af.ats[i].x+wmpbc.x;
                            wmpbc.y=af.ats[i].y+wmpbc.y;
                            wmpbc.z=af.ats[i].z+wmpbc.z;
                            */
                            // fill the vector to be aligned
                            wmsel.push_back(wmpbc);
                        }
                    } // end of the filling
                    
                    (*odtraj)<<wmsel.size()+1<<"\nAtoms\n";
                    // translate, rotate
                    pgalign(wmref, wmtg, wmsel);
                    
                    /*
                    (*odtraj)<<"N "<<wmref[0].x<<" "<<wmref[0].y<<" "<<wmref[0].z<<" "<<"\n";
                    (*odtraj)<<"H "<<wmref[1].x<<" "<<wmref[1].y<<" "<<wmref[1].z<<" "<<"\n";
                    (*odtraj)<<"H "<<wmref[2].x<<" "<<wmref[2].y<<" "<<wmref[2].z<<" "<<"\n";
                    
                    (*odtraj)<<"S "<<wmtg[0].x<<" "<<wmtg[0].y<<" "<<wmtg[0].z<<" "<<"\n";
                    (*odtraj)<<"H "<<wmtg[1].x<<" "<<wmtg[1].y<<" "<<wmtg[1].z<<" "<<"\n";
                    (*odtraj)<<"H "<<wmtg[2].x<<" "<<wmtg[2].y<<" "<<wmtg[2].z<<" "<<"\n";
                    */
                    //(*odtraj)<<"S "<<0<<" "<<0<<" "<<0<<" "<<"\n";
                    
                    for (int j=0; j<wmsel.size();++j)
                    (*odtraj)<<"N "<<wmsel[j].x<<" "<<wmsel[j].y<<" "<<wmsel[j].z<<" "<<statweight<<"\n";
        ...
        if (fgdr)
        {
            ...
            if (lgdr1=="*") al1=af.ats;
                ...
                if (chk_sel(af.ats[i].name,lgdr1)) al1.push_back(af.ats[i]);
            if (lgdr2=="*") al2=af.ats;
            else for (unsigned long i=0; i<af.ats.size(); ++i)
                if (chk_sel(af.ats[i].name,lgdr2)) al2.push_back(af.ats[i]);
        ...
        {     
            ...
            (*ogdr)<<"# Snap "<<npfr<<std::endl;
            for (unsigned long i=0; i<r.size(); ++i)
                (*ogdr)<<r[i]<<" "<<gr[i]<<std::endl;
            
            //double maxgdr=0.,rmaxgdr=0.;
            //double mingdr=1.0e+10,rmingdr=0.;
            //for (unsigned long i=0; i<r.size(); ++i) {
            //    if (r[i]>2.8 && gr[i]<mingdr) {
            //       mingdr  = gr[i];
            //       rmingdr = r[i];
            //    }
            //    if (gr[i]>maxgdr) {
            ...
            // print time,rmax,grmax,rmin,grmin 
            
            //(*ogdr)<<npfr<<" "<<rmaxgdr<<" "<<maxgdr<<" "<<rmingdr<<" "<<mingdr<<std::endl;
            
            ////(*ogdr)<<npfr<<" "<<gr[276]<<" "<<gr[331]<<std::endl;
        ...
        if (fmsdsingle)
        {   // print out the MSD for each point

            if ((npfr-1)==0){
                // initialize the matrix that will contain all the MSD
                msdnat = af.ats.size();
                DMSDSINGLE.resize(msdnat,msdlag);
            }
            msdbuff[(npfr-1)%msdlag] = af;
            //std::cout<<" \n "<<msdbuff[1].ats[0].x<<"\n";
            if((npfr-1)<(msdlag-1)) continue;
            ...
            imsd = (npfr-1)-(msdlag-1);
            for (unsigned long j=0; j<msdlag; ++j) {
                for (unsigned long o=0; o<msdnat; ++o) {
                    dx=msdbuff[(imsd+j)%msdlag].ats[o].x-msdbuff[imsd%msdlag].ats[o].x;
                    dy=msdbuff[(imsd+j)%msdlag].ats[o].y-msdbuff[imsd%msdlag].ats[o].y;
                    dz=msdbuff[(imsd+j)%msdlag].ats[o].z-msdbuff[imsd%msdlag].ats[o].z;
                    DMSDSINGLE(o,j)=DMSDSINGLE(o,j)+dx*dx+dy*dy+dz*dz;
                }
                nmsd[j]++;
                }
        }   
        ...
        if (fmsdbr)
        {
            std::valarray<double> tmphis(2);

            // resize properly the label check vector
            if (npfr==1){
                // initialize the matrix that will contain all the MSD
                fmsd_inc_br.resize(msdlag,af.ats.size()); 
                fmsd_inc_br.all()=false;
            }
            
            fmsd_inc_br.row((npfr-1)%msdlag)=false;
            if (lmsd=="*") { fmsd_inc_br.row((npfr-1)%msdlag)=true; }
            else for(unsigned long i=0; i<af.ats.size(); ++i)
                if(af.ats[i].name==lmsd) { 
                    fmsd_inc_br((npfr-1)%msdlag,i)=true;
                }
            ...
            for (unsigned long j=0; j<msdlag; ++j) {
                double cmsd=0.;
                for (unsigned long o=0; o<af.ats.size(); ++o)
                    if(fmsd_inc_br(imsd%msdlag,o)) {
                       dx=msdbuff[(imsd+j)%msdlag].ats[o].x-msdbuff[imsd%msdlag].ats[o].x;
                       dy=msdbuff[(imsd+j)%msdlag].ats[o].y-msdbuff[imsd%msdlag].ats[o].y;
                       dz=msdbuff[(imsd+j)%msdlag].ats[o].z-msdbuff[imsd%msdlag].ats[o].z;
                       //cmsd+=(dx*dx+dy*dy+dz*dz)/msdnat[imsd%msdlag];
                       tmphis[0]=j*dt; tmphis[1]=dx*dx+dy*dy+dz*dz;
                       hgbr.add(tmphis,1.0);
                       dmsd[j]+=tmphis[1];
                       nmsd[j]++;
                    }                
    ...
    if (fmsd)
    {   
        // finishing to avarage what I still can avarage
        for (unsigned long ts=0; ts<msdlag; ++ts) {
            ++imsd;
            std::cerr<<  "Finishing to avarage the MSD "<<std::setw(10)<<std::setiosflags(std::ios::right)<<imsd<<"\r";
            for (unsigned long j=0; j<msdlag; ++j) {
        ...
        for (unsigned long ts=0; ts<msdlag; ++ts) {
            ...
            for (unsigned long j=0; j<msdlag; ++j) {
                    ...
                    for (unsigned long o=0; o<msdnat; ++o) {
                        ...
                        dy=msdbuff[(imsd+j)%msdlag].ats[o].y-msdbuff[imsd%msdlag].ats[o].y;
                        dz=msdbuff[(imsd+j)%msdlag].ats[o].z-msdbuff[imsd%msdlag].ats[o].z;
                        DMSDSINGLE(o,j)+=dx*dx+dy*dy+dz*dz;
                    }
                    nmsd[j]++;
                    }
        }
        
        // Time to print out the files
        
        for (unsigned long o=0; o<msdnat; ++o){
            std::cerr<<"# PRINTING OUT msds "<<o<<"\r";
            std::ostringstream convert;   // stream used for the conversion
            convert << o;
            omsd=new std::ofstream((prefix+std::string("-")+convert.str()+std::string(".msd")).c_str());
            (*omsd).precision(8); (*omsd).width(15); (*omsd).setf(std::ios::scientific);
            for (unsigned long it=0; it<msdlag; ++it)
            {
                (*omsd) <<it*dt<<"  "<<DMSDSINGLE(o,it)/nmsd[it]<<std::endl;
            }
        }
        std::cerr<<"# FINISHED!\n";
    }
    ...
    {   
        std::valarray<double> tmphis(2);
        //std::cerr<<"suca!\n";
        // finishing to average what I still can average
        ...
        for (unsigned long ts=0; ts<msdlag; ++ts) {
            ...
            for (unsigned long j=0; j<msdlag; ++j) {
                    if ((imsd+j)>(npfr-1)) break;
                    for (unsigned long o=0; o<msdbuff[imsd%msdlag].ats.size(); ++o) {
                        if(fmsd_inc_br(imsd%msdlag,o)) {
                           dx=msdbuff[(imsd+j)%msdlag].ats[o].x-msdbuff[imsd%msdlag].ats[o].x;
                           dy=msdbuff[(imsd+j)%msdlag].ats[o].y-msdbuff[imsd%msdlag].ats[o].y;
                           dz=msdbuff[(imsd+j)%msdlag].ats[o].z-msdbuff[imsd%msdlag].ats[o].z;
        ...
        std::cerr<<"# PRINTING OUT msdbrow\n";
        ind = 0;
        for (ind[0]=0; ind[0]<hgobr[0].boundaries.size()-1; ++ind[0])
        {
           for (ind[1]=0; ind[1]<hgobr[1].boundaries.size()-1; ++ind[1])
           {
              hgbr.get_bin(ind,cen,val);
              
              (*omsdbr) <<cen[0]<<" "<<cen[1]<<" "<<val<<"\n";
           }
